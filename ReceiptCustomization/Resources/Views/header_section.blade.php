<div style="border:1px solid #000; padding:10px; font-family: Arial, sans-serif; font-size:11px;">
        ...
        <div style="width:40%; text-align:right;">
            ...
            <table style="width:100%; font-size:11px; border-collapse:collapse;">
                ...
                <tr>
                    ...
                    <td style="text-align:left; padding:2px 4px; border:1px solid #ccc;">{{ $order->user->username ?? '—' }}</td>
                </tr>
                <tr>
    ...
    <div style="margin-bottom:8px;">
        <table style="width:100%; font-size:11px; border-collapse:collapse;">
            <tr>
                ...
                <td style="padding:2px 4px; border:1px solid #ccc; width:34%;" colspan="3">
                    {{ trim( ( $order->vehicle_year ?? '' ) . ' ' . ( $order->vehicle_make ?? '' ) . ' ' . ( $order->vehicle_model ?? '' ) ) ?: '—' }}
                </td>
                <td style="padding:2px 4px; border:1px solid #ccc; font-weight:bold; width:16%;">{{ __( 'Color' ) }}</td>
                <td style="padding:2px 4px; border:1px solid #ccc; width:34%;">{{ $order->vehicle_color ?? '—' }}</td>
            </tr>
            <tr>
    ...
    --}}
    @if( ! empty( $order->stock_no ) || ! empty( $order->terms ) || ! empty( $order->representative ) || ! empty( $order->selling_dealer ) || ! empty( $order->customer_telephone ) || ! empty( $order->customer_fax ) )
    <div style="margin-bottom:8px;">
        <table style="width:100%; font-size:11px; border-collapse:collapse;">
            <tr>
                <td style="padding:2px 4px; border:1px solid #ccc; font-weight:bold; width:16%;">{{ __( 'Selling Dealer' ) }}</td>
                <td style="padding:2px 4px; border:1px solid #ccc; width:34%;">{{ $order->selling_dealer ?? '—' }}</td>
            ...
            <tr>
                <td style="padding:2px 4px; border:1px solid #ccc; font-weight:bold;">{{ __( 'Representative' ) }}</td>
                <td style="padding:2px 4px; border:1px solid #ccc;">{{ $order->representative ?? '—' }}</td>
            ...
            @if( ! empty( $order->customer_telephone ) || ! empty( $order->customer_fax ) )
            <tr>
                ...
                <td style="padding:2px 4px; border:1px solid #ccc;">{{ $order->customer_fax ?? '—' }}</td>
            </tr>
            @endif
        </table>
    </div>
